...
The task dashboard supports list/grid views, priority-aware manual drag-and-drop ordering, priority/due-date/created-date sorting, tag filters, owner filters, and due-date filters such as due today, due this week, past due, no date, and completed. Drag the grip or move the desktop task card by at least 4px and the dashboard automatically switches to **Sort: Manual**; normal clicks still open Task Details. FocusClaw captures from mouse-down and adds a window-level fallback so one-step fast gestures cannot outrun activation or release. The final release position determines the drop, gaps snap to the nearest valid same-priority card, and list collision follows the dragged card's leading edge for earlier vertical snapping. Switching list/grid safely resets any unfinished gesture. Reorder saves run sequentially in the background without locking later drags. On mobile, a 0.5-second press anywhere on a task also enters Manual sort and starts a full-card drag; ordinary taps and scrolling remain unchanged. The GPU-accelerated whole-card preview follows the pointer smoothly on desktop and touch devices. Priority groups stay in priority order, while tasks can be placed anywhere inside their own priority group. Manual order persists across refreshes and works in single-project and **Project: All** views.
